...
def _strip_grid_lines(cell, min_line_ratio=0.5, search_band=10):
    """
    edge_offset이 0이 아니어도(경계선이 칸 경계에서 몇 픽셀 안쪽에서
    시작해도), search_band 범위 안에서 가장 유력한 격자선 위치를 찾는다.
    "0번째부터 연속"을 요구하지 않으므로 스캔 정렬 오차나 기울어짐에
    더 강건하다.
    """
    inv = 255 - cell
    h, w = inv.shape

    row_ratio = (inv > 0).sum(axis=1) / w
    col_ratio = (inv > 0).sum(axis=0) / h

    def _find_cut(ratios, limit):
        # limit 범위 안에서 기준을 넘는 마지막 위치까지 지운다
        # (중간에 기준 미달 구간이 있어도 이후에 다시 선이 나오면 계속 포함)
        cut = 0
    ...
    top = _find_cut(row_ratio, search_band)
    bottom = _find_cut(row_ratio[::-1], search_band)
    left = _find_cut(col_ratio, search_band)
    right = _find_cut(col_ratio[::-1], search_band)
