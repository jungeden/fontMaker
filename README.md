...
- **굵기**: 벡터 도형을 확대/축소하면 획 굵기도 같이 확대/축소되기 때문에, 자모/문자마다
  ...
  `config.py`의 `TARGET_STROKE_PX`). 단순한 획(직선 등)에는 잘 맞지만, ㄱ처럼 꺾이는
  부분이 있는 복잡한 모양은 추정치가 다소 부정확할 수 있다 — 완벽한 굵기 통일보다는
  "차이를 크게 줄이는" 수준으로 이해하면 된다.
...
- 획 굵기 자동 보정은 잉크 면적/둘레 비율 기반의 근사치라, 꺾이는 부분이 많은 복잡한
  모양에서는 완벽하게 균일하지 않을 수 있다.
